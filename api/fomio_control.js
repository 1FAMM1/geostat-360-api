...
async function handleGetTeams(req, res) {
  const { data: teams, error } = await supabase
    .from('fomio_teams')
    .select('id, team_name, patente, nome, n_int, h_entrada, h_exit, observ, MP, TAS')
...
async function handleInsertMember(req, res) {
  ...
  const { data, error } = await supabase
    .from('fomio_teams')
    .insert([{ team_name, patente, nome, n_int, h_entrada, h_exit, observ, MP, TAS }])
    .select();
...
// As funções handleClearAll, handleResetSequence, handleSaveHeader, handleGetHeader e handleLegacyRouting
// permanecem exatamente iguais à versão original
